...
var (
	baseURL         = "wss://stream.binancezh.com:9443/ws"
	combinedBaseURL = "wss://stream.binancezh.com:9443/stream?streams="
...
// WsTradeHandler handle websocket binance event
...
func WsTradeServe(symbol string, handler WsTradeHandler, errHandler ErrHandler) (doneC, stopC chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/%s@binance", baseURL, strings.ToLower(symbol))
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
...
// WsTradeEvent define websocket binance event
